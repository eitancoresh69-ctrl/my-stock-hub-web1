    ...
    if sel:
        with st.spinner(f"מושך נתונים פיננסיים בזמן אמת עבור {sel} משרתי Yahoo Finance..."):
            ...
            if not info or financials.empty:
                ...
                st.success(f"✅ נתונים נטענו בהצלחה עבור: **{info.get('longName', sel)}**")
                
                # --- מדדי מפתח ---
                st.subheader("💡 מדדי מפתח (Key Metrics)")
                c1, c2, c3, c4 = st.columns(4)
                
                market_cap = info.get('marketCap', 0) / 1e9 # במיליארדים
                pe_ratio = info.get('trailingPE', 'N/A')
                profit_margin = info.get('profitMargins', 0) * 100
                rev_growth = info.get('revenueGrowth', 0) * 100
                
                c1.metric("שווי שוק", f"${market_cap:.2f}B" if market_cap else "N/A")
                c2.metric("מכפיל רווח (P/E)", f"{pe_ratio:.2f}" if isinstance(pe_ratio, float) else pe_ratio)
                ...
                try:
                    # חילוץ נתונים מהדוחות של יאהו (לוקח את 4 השנים האחרונות)
                    ...
                    st.info("נתוני ההכנסות השנתיים חסרים או לא בפורמט צפוי.")

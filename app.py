...
st.markdown("""
    ...
    .ai-card { background: linear-gradient(145deg, #ffffff, #f0f7ff); padding: 15px; border-radius: 12px; border-right: 6px solid #1a73e8; box-shadow: 0 4px 10px rgba(0,0,0,0.08); margin-bottom: 15px; }
...
gold_count = len(df_all[df_all["Score"] >= 5]) if not df_all.empty else 0
c2.metric("🏆 מניות זהב (ציון 5-6)", gold_count, help="כמות המניות שעברו את הסינון הקפדני.")

top_gainer = df_all.loc[df_all["Change"].idxmax()]["Symbol"] if not df_all.empty else "N/A"
c3.metric("🔥 המזנקת היומית", top_gainer)
...
with tab2:
    ...
    if not df_all.empty:
        ...
        st.warning("הנתונים נטענים. אם הודעה זו נשארת, ייתכן ויש עומס על שרתי Yahoo Finance.")
...
with tab4:
    if not df_all.empty:
        ...
        try:
            ...
            st.error("לא ניתן לטעון גרף כרגע.")

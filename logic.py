...
def evaluate_pdf_metrics(info):
    ...
    try:
        ...
        details['EarnGrowth'] = earn_growth or 0
...
def get_ai_logic(price, fv, score, currency):
    ...
    if score >= 5:
        ...
        if gap > 0.10: return "איסוף 🛒", f"חברה טובה שנסחרת במחיר 'מבצע' מתחת לשווי של {currency}{fv:,.2f}."
        return "החזק ⚖️", "החברה יציבה אך המחיר משקף את השווי האמיתי."
    return "מכירה/המתנה 🔴", "ציון איכות נמוך יחסית לסיכון (לפי מדדי ה-PDF)."
